...
fn main() {
    ...
    let mut model: metaformer::MetaFormer = metaformer(
        vs_path,
        config.dimension,
    ...
    let mut opt: Optimizer = {
        ...
        let adam: Result<Optimizer, TchError> = tch::nn::Adam::default()
    ...
    for eval_idx in 0..config.slices {
        let mut loss_accumulator = MetricAccumulator::new("loss/test");
        let mut acc_accumulator = MetricAccumulator::new("acc/test");

        let (avg_train_loss, avg_acc) = {
            
            let dataslice: std::collections::HashMap<String, tch::Tensor> = read_dataslice(-eval_idx);
            let x_sc = dataslice.get("X").unwrap().to(training_device);
            let y_s = dataslice.get("Y").unwrap().to(training_device);
            println!("Loaded slice to device.");
            let s = y_s.size()[0]; // slice size s
            ...
            for idx in 0..(s / config.batch_size) {
                ...
                loss_accumulator.accumulate(loss.double_value(&[]));
                acc_accumulator.accumulate(loss.double_value(&[]));
        ...
        log_metrics(&config, vec![avg_acc, avg_train_loss]);
