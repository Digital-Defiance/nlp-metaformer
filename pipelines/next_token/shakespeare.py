...
class ShakeSpeareData(LightningDataModule):
    ...
    def data_loader(self, split: Literal["train", "val"]):
        ...
        iterations = 100 if split == "train" else 1
        block_size = 50
        batch_size = 10
        def make_iterator():
            for _ in range(iterations):
                ...
                yield x_bcd.to(DEVICE), y_bcd.to(DEVICE)
...
def train_shakespear(
    ...
    data_module = ShakeSpeareData()
    model = NextTokenPredictor(cfg)
    print(model)
    trainer = Trainer(max_epochs=10000, barebones=True)
